...
run_lane() {
    ...
    run_optional_tool network "dig dns lookup" "bind-tools|dnsutils" dig "dig +time=2 +tries=1 example.com A >/tmp/cli-corner-smoke/dig.out 2>&1 && grep -Eq '^[^;].*[[:space:]]A[[:space:]]+[0-9]' /tmp/cli-corner-smoke/dig.out && echo dig-ok" "dig-ok"
