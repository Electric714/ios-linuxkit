...
Phase 4 dormant trace-record guard-validation tranche:

- Added a non-executing fail-closed `arm64_trace_record` guard validator and counters. The validator checks the hot-trace gate, armed/not-retired state, source/target liveness, matching `invalidate_gen`, same-page ownership, recorded source/target PCs and adjacent target shape, exact source-slot value, and non-null target gadget pointer. It is exercised only from record creation/duplicate diagnostics and does not enter dispatch, emit trace code, add exits, or change `jump_ip`/ret-cache/inline-chain semantics.
- Extended `ARM64_BLOCK_HOT_STATS` with `hot_trace_record_guard_*` counters for checks, passes, and fail reasons. The sidecar fixture now asserts guard checks remain zero with stats-only diagnostics and that all guard checks pass in the stats+hot-trace row. Report `/workspace/tmp/ish-arm64-hot-trace-record-smoke-20260517-143334.md` was **7 / 7 passing** with `hot_trace_record_guard_checks=8192` and `hot_trace_record_guard_passes=8192`.
- Validation after guard counters: default Node/Bun `/workspace/tmp/ish-arm64-node-bun-perf-20260517-143358.md` and stats+hot-trace Node/Bun `/workspace/tmp/ish-arm64-node-bun-perf-20260517-143433.md` were both **10 / 10 passing**; default Alpine runtime coverage `/workspace/tmp/ish-arm64-runtime-coverage-20260517-144804.md` was **83 / 83 passing**. An initial coverage retry at `/workspace/tmp/ish-arm64-runtime-coverage-20260517-143511.md` hit a transient Java VM SIGSEGV/timer expiry and was not used as the gate.
