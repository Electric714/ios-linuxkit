...
| CLI corner-case smoke | Passing, Alpine 27/27 | Optional/diagnostic harness for CLI/TUI/network/container tools likely to expose terminal, `/proc`, DNS, ptrace, netlink, Unix-socket, and package-manager edge cases. Covers `nushell`/`xonsh` availability, `htop` and `btop` inside `tmux`, `tcpdump`, `traceroute`, `iproute2`, `bind-tools`, direct `curl`/HTTPS `git` DNS, `rcarmo/go-gte` clone, Docker CLI `hello-world`, `strace`, `lsof`, `file`, `jq`, and Linuxbrew availability. | `/workspace/tmp/ish-arm64-cli-corner-smoke-20260516-171044.md` |
...
```text
27 / 27 passing
report: /workspace/tmp/ish-arm64-cli-corner-smoke-20260516-171044.md
```
...
- `docker run --rm hello-world` is represented as a diagnostic row. In the Alpine fakefs the Docker CLI starts correctly, but there is no Docker daemon/socket, so the passing outcome records the explicit daemon-unavailable diagnostic rather than pretending a container ran.
