...
The staged runtime suite includes `arm64 barriers DMB/DSB/ISB`, which compiles and executes common barrier encodings (`dmb sy`, `dmb ish`, `dmb ishld`, `dmb ishst`, `dsb sy`, `dsb ish`, and `isb`) inside the guest. Latest staged coverage is `/workspace/tmp/ish-arm64-runtime-coverage-20260513-220623.md` with **49 / 49 passing**.
...
The staged runtime suite now includes additional language/toolchain smoke or availability coverage and validates **49 / 49 passing** in `/workspace/tmp/ish-arm64-runtime-coverage-20260513-220623.md`:
